/**
 * Created by madhukar on 04.03.15.
 */
var mc_table;
...
$(document).ready(function(){
    ...
    var json_file_location = 'LoanDetails.json';
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            for (var i = 0; i < data.length; i++) {
                loanDetailsSet.push([data[i].UserName, data[i].AccountNumber, data[i].TransactionNumber, data[i].Deposit, data[i].TotalSaving ,data[i].MonthlyEMI, data[i].EMITotalBalance, data[i].PaymentMonth]); //
            }
        },
        error:function(xhr){
    ...
    mc_table = $('#loan_details').dataTable( {
        "data": loanDetailsSet,
        "order": [[7, 'desc']],
